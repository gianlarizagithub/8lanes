...
export class FormComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.currentuserloggedinobject =
      this.authService.getCurrentUserLoggedInObject();
  }
  ...
  async retrievSuiffxNameLogic() {
    ...
    this.suffixnamesList = this.suffixnamesList.sort((a, b) => {
      return a.name < b.name ? -1 : 0;
    });
  }
  async retrieveSuffixName() {
    const retrievesuffixnameresponse = await fetch(
      'assets/suffixname/suffixnames.json'
    );
    return await retrievesuffixnameresponse.json();
  }
  async retrievePhil_Prov_Mun_Bar() {
    const retrievePhil_Prov_Mun_BarResponse = await fetch(
      'assets/phil&prov&mun&bar/phil_prov_mun_bar.json'
    );
    return await retrievePhil_Prov_Mun_BarResponse.json();
  }
  async retrieveRegionProvinceCityBarangayLogic() {
    const retrievePhil_Prov_Mun_BarList =
      await this.retrievePhil_Prov_Mun_Bar();
    ...
    for (let [key, value] of Object.entries(retrievePhil_Prov_Mun_BarList)) {
      ...
      getretrievePhil_Prov_Mun_BarListObjectEntries.push({
        key: getretrievePhil_Prov_Mun_BarListObjectEntriesValue,
      });
    }
    ...
    this.retrievePhil_Prov_Mun_BarList =
      getretrievePhil_Prov_Mun_BarListObjectEntriesValueList.sort((a, b) => {
        return a.region_name < b.region_name ? -1 : 0;
      });

    var selectRegionOnChangeIdAttribute = document.getElementById(
      'selectRegion'
    ) as any;

    var selectProvinceOnChangeIdAttribute = document.getElementById(
      'selectProvince'
    ) as any;

    var selectCityOnChangeIdAttribute = document.getElementById('selectCity');

    await selectRegionOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        ...
        if (RegionEventValue == '') {
          ...
          this.retrieveProvinceFromRegion =
            this.retrievePhil_Prov_Mun_BarList.filter(
              (f: any) => f.region_name == RegionEventValue
            );
          this.retrieveProvinceFromRegion = this.retrieveProvinceFromRegion.map(
            (e: any) => {
              return e.province_list;
            }
          );
          this.retrieveProvinceFromRegion = this.retrieveProvinceFromRegion.map(
            (e: any) => {
          ...
          this.retrieveProvinceFromRegion = Object.keys(
            this.retrieveProvinceFromRegion[0]
          );
          this.retrieveProvinceFromRegionLength =
            this.retrieveProvinceFromRegion.length;
          this.retrieveCityFromProvince = [];
          this.f['province'].setValue('');
          this.retrieveCityFromProvinceLength = 0;
          this.f['city'].setValue('');
    ...
    await selectProvinceOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        ...
        if (ProvinceEventValue == '') {
          ...
          this.retrieveCityFromProvince =
            this.retrievePhil_Prov_Mun_BarList.filter(
              (f: any) =>
                f.region_name == selectRegionOnChangeIdAttribute?.value
            );
          this.retrieveCityFromProvince = this.retrieveCityFromProvince.map(
            (e: any) => {
              return e.province_list;
            }
          );
          this.retrieveCityFromProvince = this.retrieveCityFromProvince.map(
            (e: any) => {
              return e[ProvinceEventValue].municipality_list;
            }
          );
          this.retrieveCityFromProvince = Object.keys(
            this.retrieveCityFromProvince[0]
          );
          this.retrieveCityFromProvinceLength =
            this.retrieveCityFromProvince.length;
          this.retrieveBarangayFromCity = [];
          this.retrieveBarangayFromCityLength = 0;
          this.f['city'].setValue('');
          this.f['barangay'].setValue('');
    ...
    await selectCityOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        ...
        if (CityEventValue == '') {
          this.retrieveBarangayFromCity = [];
          this.retrieveBarangayFromCityLength =
            this.retrieveBarangayFromCity.length;
          this.f['barangay'].setValue('');
        } else {
          this.f['barangay'].setValue('');
          this.retrieveBarangayFromCity =
          ...
          this.retrieveBarangayFromCity = this.retrieveBarangayFromCity.map(
            (e: any) => {
              return e.province_list;
            }
          );
          this.retrieveBarangayFromCity = this.retrieveBarangayFromCity.map(
            (e: any) => {
              return e[selectProvinceOnChangeIdAttribute?.value]
                .municipality_list;
            }
          );
          this.retrieveBarangayFromCity = this.retrieveBarangayFromCity.map(
            (e: any) => {
              return e[CityEventValue];
            }
          );
          this.retrieveBarangayFromCity =
            this.retrieveBarangayFromCity[0].barangay_list;
          this.retrieveBarangayFromCityLength =
            this.retrieveBarangayFromCity.length;
        }
      }
  ...
  async retrieveCourseLogic() {
    ...
    this.courseList = this.courseList.sort((a, b) => {
      return a.CourseName < b.CourseName ? -1 : 0;
    });
  }
  onSubmit() {
  ...
  async retrieveTransmission() {
    const retrieveTransmissionResponse = await fetch(
      'assets/transmission/transmission.json'
    );
  ...
  async retrieveTransmissionLogic() {
    ...
    this.transmissionList = this.transmissionList.sort((a, b) => {
      return a.Type < b.Type ? -1 : 0;
    });
  }
  async retrieveTypeofVehicle() {
    const retrieveTypeofVehicleResponse = await fetch(
      'assets/vehicle/vehicle.json'
    );
  ...
  async retrieveTypeofVehicleLogic() {
    ...
    this.typeofvehicleList = this.typeofvehicleList.sort((a, b) => {
      return a.Type < b.Type ? -1 : 0;
    });
  }
  async retrieveRestrictionCode() {
    const retrieveRestrictionCodeResponse = await fetch(
      'assets/restrictioncode/restrictioncode.json'
    );
  ...
  async retrieveRestrictionCodeLogic() {
    ...
    if (
      this.f['typeofvehicle'].value == '' ||
      this.f['typeofvehicle'].value == null ||
      this.f['typeofvehicle'].value == undefined
    ) {
      this.f['restrictioncode'].setValue('');
      this.restrictioncodeList = [];
      this.restrictioncodeListLength = 0;
    } else {
      this.restrictioncodeList = this.restrictioncodeList.filter(
        (f) => f.Type == this.f['typeofvehicle'].value
      );
      this.restrictioncodeList = this.restrictioncodeList.sort((a, b) => {
  ...
  typeofvehicleOnChangeEvent() {
    var selectinputTypeofVehicleOnChangeIdAttribute = document.getElementById(
      'selectinputTypeofVehicle'
    );

    selectinputTypeofVehicleOnChangeIdAttribute?.addEventListener(
      'change',
      async (event: any) => {
  ...
  async retrieveNationalities() {
    const retrieveNationalitiesResponse = await fetch(
      'assets/nationalities/nationalities.json'
    );
  ...
  async retrieveNationalitiesLogic() {
    ...
    this.retrieveNationalitiesList = this.retrieveNationalitiesList.sort(
      (a, b) => {
        return a < b ? -1 : 0;
      }
    );
  ...
  async retrieveCompanies() {
    const retrieveCompaniesResponse = await fetch(
      'assets/companies/companies.json'
    );
    return await retrieveCompaniesResponse.json();
  }
  async retrieveOccupations() {
    const retrieveOccupationsResponse = await fetch(
      'assets/occupation/occupation.json'
    );
  ...
  async retrieveCompaniesLogic() {
    ...
    this.retrieveCompaniesList = this.retrieveCompaniesList.sort((a, b) => {
      return a < b ? -1 : 0;
    });
  }
  async retrieveOccupationsLogic() {
    this.retrieveOccupationsList = await this.retrieveOccupations();
    this.retrieveOccupationsList = this.retrieveOccupationsList.sort((a, b) => {
      return a < b ? -1 : 0;
    });
  }
  ...
  async retrieveBirthplaceRegionProvinceCityBarangayLogic() {
    ...
    this.retrieveBirthplacePhil_Prov_Mun_BarList =
      getretrievePhil_Prov_Mun_BarListObjectEntriesValueList.sort((a, b) => {
        return a.region_name < b.region_name ? -1 : 0;
      });

    var selectBirthplaceRegionOnChangeIdAttribute = document.getElementById(
      'selectbirthplaceregion'
    ) as any;

    var selectBirthplaceProvinceOnChangeIdAttribute = document.getElementById(
      'selectbirthplaceprovince'
    ) as any;

    var selectBirthplaceCityOnChangeIdAttribute = document.getElementById(
      'selectbirthplacecity'
    );

    await selectBirthplaceRegionOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        const RegionEventValue = event.target.value;
        ...
        if (RegionEventValue == '') {
          ...
          this.retrieveBirthplaceProvinceFromRegion =
            this.retrieveBirthplacePhil_Prov_Mun_BarList.filter(
              (f: any) => f.region_name == RegionEventValue
            );
          this.retrieveBirthplaceProvinceFromRegion =
            this.retrieveBirthplaceProvinceFromRegion.map((e: any) => {
              return e.province_list;
            });
          this.retrieveBirthplaceProvinceFromRegion =
            this.retrieveBirthplaceProvinceFromRegion.map((e: any) => {
          ...
          this.retrieveBirthplaceProvinceFromRegion = Object.keys(
            this.retrieveBirthplaceProvinceFromRegion[0]
          );
          this.retrieveBirthplaceProvinceFromRegionLength =
            this.retrieveBirthplaceProvinceFromRegion.length;
          this.retrieveBirthplaceCityFromProvince = [];
          this.f['birthplaceprovince'].setValue('');
          this.retrieveBirthplaceCityFromProvinceLength = 0;
          this.f['birthplacecity'].setValue('');
    ...
    await selectBirthplaceProvinceOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        const ProvinceEventValue = event.target.value;
        ...
        if (ProvinceEventValue == '') {
          ...
          this.retrieveBirthplaceCityFromProvince =
            this.retrieveBirthplacePhil_Prov_Mun_BarList.filter(
              (f: any) =>
                f.region_name ==
                selectBirthplaceRegionOnChangeIdAttribute?.value
            );
          this.retrieveBirthplaceCityFromProvince =
            this.retrieveBirthplaceCityFromProvince.map((e: any) => {
              return e.province_list;
            });
          this.retrieveBirthplaceCityFromProvince =
            this.retrieveBirthplaceCityFromProvince.map((e: any) => {
          ...
          this.retrieveBirthplaceCityFromProvince = Object.keys(
            this.retrieveBirthplaceCityFromProvince[0]
          );
          this.retrieveBirthplaceCityFromProvinceLength =
            this.retrieveBirthplaceCityFromProvince.length;
          this.retrieveBirthplaceBarangayFromCity = [];
          this.retrieveBirthplaceBarangayFromCityLength = 0;
          this.f['birthplacecity'].setValue('');
          this.f['birthplacebarangay'].setValue('');
    ...
    await selectBirthplaceCityOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        const CityEventValue = event.target.value;
        ...
        if (CityEventValue == '') {
          this.retrieveBirthplaceBarangayFromCity = [];
          this.retrieveBirthplaceBarangayFromCityLength =
            this.retrieveBirthplaceBarangayFromCity.length;
          this.f['birthplacebarangay'].setValue('');
        } else {
          this.f['birthplacebarangay'].setValue('');
          this.retrieveBirthplaceBarangayFromCity =
          ...
          this.retrieveBirthplaceBarangayFromCity =
            this.retrieveBirthplaceBarangayFromCity.map((e: any) => {
              return e.province_list;
            });
          this.retrieveBirthplaceBarangayFromCity =
            this.retrieveBirthplaceBarangayFromCity.map((e: any) => {
              return e[selectBirthplaceProvinceOnChangeIdAttribute?.value]
                .municipality_list;
            });
          this.retrieveBirthplaceBarangayFromCity =
            this.retrieveBirthplaceBarangayFromCity.map((e: any) => {
          ...
          this.retrieveBirthplaceBarangayFromCity =
            this.retrieveBirthplaceBarangayFromCity[0].barangay_list;
          this.retrieveBirthplaceBarangayFromCityLength =
            this.retrieveBirthplaceBarangayFromCity.length;
        }
      }
  ...
  async retrieveParentGurdianRegionProvinceCityBarangayLogic() {
    ...
    this.retrieveParentGurdianPhil_Prov_Mun_BarList =
      getretrievePhil_Prov_Mun_BarListObjectEntriesValueList.sort((a, b) => {
        return a.region_name < b.region_name ? -1 : 0;
      });

    var selectParentGurdianRegionOnChangeIdAttribute = document.getElementById(
      'selectParentRegion'
    ) as any;

    var selectParentGurdianProvinceOnChangeIdAttribute =
      document.getElementById('selectParentProvince') as any;

    var selectParentGurdianCityOnChangeIdAttribute =
      document.getElementById('selectParentCity');

    await selectParentGurdianRegionOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        const RegionEventValue = event.target.value;
        ...
        if (RegionEventValue == '') {
          ...
          this.retrieveParentGurdianProvinceFromRegion =
            this.retrieveParentGurdianPhil_Prov_Mun_BarList.filter(
              (f: any) => f.region_name == RegionEventValue
            );
          this.retrieveParentGurdianProvinceFromRegion =
            this.retrieveParentGurdianProvinceFromRegion.map((e: any) => {
              return e.province_list;
            });
          this.retrieveParentGurdianProvinceFromRegion =
            this.retrieveParentGurdianProvinceFromRegion.map((e: any) => {
          ...
          this.retrieveParentGurdianProvinceFromRegion = Object.keys(
            this.retrieveParentGurdianProvinceFromRegion[0]
          );
          this.retrieveParentGurdianProvinceFromRegionLength =
            this.retrieveParentGurdianProvinceFromRegion.length;
          this.retrieveParentGurdianCityFromProvince = [];
          this.f['parentprovince'].setValue('');
          this.retrieveParentGurdianCityFromProvinceLength = 0;
          this.f['parentcity'].setValue('');
    ...
    await selectParentGurdianProvinceOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        const ProvinceEventValue = event.target.value;
        ...
        if (ProvinceEventValue == '') {
          ...
          this.retrieveParentGurdianCityFromProvince =
            this.retrieveParentGurdianPhil_Prov_Mun_BarList.filter(
              (f: any) =>
                f.region_name ==
                selectParentGurdianRegionOnChangeIdAttribute?.value
            );
          this.retrieveParentGurdianCityFromProvince =
            this.retrieveParentGurdianCityFromProvince.map((e: any) => {
              return e.province_list;
            });
          this.retrieveParentGurdianCityFromProvince =
            this.retrieveParentGurdianCityFromProvince.map((e: any) => {
          ...
          this.retrieveParentGurdianCityFromProvince = Object.keys(
            this.retrieveParentGurdianCityFromProvince[0]
          );
          this.retrieveParentGurdianCityFromProvinceLength =
            this.retrieveParentGurdianCityFromProvince.length;
          this.retrieveParentGurdianBarangayFromCity = [];
          this.retrieveParentGurdianBarangayFromCityLength = 0;
          this.f['parentcity'].setValue('');
          this.f['parentbarangay'].setValue('');
    ...
    await selectParentGurdianCityOnChangeIdAttribute?.addEventListener(
      'change',
      (event: any) => {
        const CityEventValue = event.target.value;
        ...
        if (CityEventValue == '') {
          this.retrieveParentGurdianBarangayFromCity = [];
          this.retrieveParentGurdianBarangayFromCityLength =
            this.retrieveParentGurdianBarangayFromCity.length;
          this.f['parentbarangay'].setValue('');
        } else {
          this.f['parentbarangay'].setValue('');
          this.retrieveParentGurdianBarangayFromCity =
          ...
          this.retrieveParentGurdianBarangayFromCity =
            this.retrieveParentGurdianBarangayFromCity.map((e: any) => {
              return e.province_list;
            });
          this.retrieveParentGurdianBarangayFromCity =
            this.retrieveParentGurdianBarangayFromCity.map((e: any) => {
              return e[selectParentGurdianProvinceOnChangeIdAttribute?.value]
                .municipality_list;
            });
          this.retrieveParentGurdianBarangayFromCity =
            this.retrieveParentGurdianBarangayFromCity.map((e: any) => {
          ...
          this.retrieveParentGurdianBarangayFromCity =
            this.retrieveParentGurdianBarangayFromCity[0].barangay_list;
          this.retrieveParentGurdianBarangayFromCityLength =
            this.retrieveParentGurdianBarangayFromCity.length;
        }
      }
